...
(function (global) {
  ...
  function removeAllForContact(contactId) {
    ...
    var data = loadRaw();
    var changed = false;
    if (data.roomSettings[id]) {
      delete data.roomSettings[id];
      changed = true;
    }
    if (data.activeSessions[id]) {
      delete data.activeSessions[id];
      changed = true;
    }
    var beforeLen = data.favorites.length;
    data.favorites = data.favorites.filter(function (f) { return !(f && f.contactId === id); });
    if (data.favorites.length !== beforeLen) changed = true;
    if (changed) saveRaw();
    return changed;
  }
