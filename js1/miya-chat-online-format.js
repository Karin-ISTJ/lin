(function (global) {
    ...
    function buildOnlineRules(opts) {
        ...
        var rows = [
            ...
            '5.1、【看不到画面时必须承认】当用户发来的图片在上下文里显示为「[图片：内容未知]」时，表示你（角色）没有看到这张图的画面内容。此时：可以确认「对方发了张图」这件事本身，也可以询问图里是什么；但禁止描述、猜测或编造画面内容（人物、场景、物体、文字等一律不许写）。宁可说「我这边刷不出来」「看不清」，也不要自行补全。',
    ...
    function formatMessageForApi(m) {
        ...
        if (m.type === 'image') {
            var imgBody = '';
            /*
             * 真实照片但还没识图（识图失败 / 尚未跑 / 模型不支持图片）：不能只留一句「[图片]」。
             * 留白等于告诉模型「这里有图、你自己看着办」——它会顺势编造画面内容，
             * 而角色其实根本没看到。所以这里显式声明「内容未知」，
             * 把「不确定」本身当作信息交出去，堵住脑补。
             * 参照摄像头已关闭那条同构规则（engine 里的「用户摄像头状态」）。
             */
            var imgUnknown = false;
            if (isRealChatPhotoMessage(m)) {
                imgBody = trim(m.imageVisionText);
                if (!imgBody) imgBody = trim(m.content);
                if (imgBody === '[图片]') imgBody = '';
                /* 识图没拿到文本 → 角色看不到画面内容 */
                if (!imgBody) imgUnknown = true;
            } else {
