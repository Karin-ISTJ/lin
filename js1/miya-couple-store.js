...
(function (global) {
  ...
  function closeSpace(contactId) {
    var id = String(contactId || '').trim();
    if (!id) return false;
    var data = loadRaw();
    var existed = !!data.spaces[id];
    var changed = existed;
    if (existed) delete data.spaces[id];
    Object.keys(data.invites).forEach(function (invId) {
      var inv = data.invites[invId];
      if (inv && String(inv.contactId || '').trim() === id) {
        delete data.invites[invId];
        changed = true;
      }
    });
    if (changed) saveRaw();
