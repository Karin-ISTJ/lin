...
(function (global) {
  ...
  function confirmDialog(opts) {
    ...
    return Promise.resolve(global.confirm(String((opts.title || '') + '\n' + (opts.message || ''))));
  }

  function unbindSpace(contactId) {
    var cid = contactId || state.activeSpaceContactId;
    if (!cid) return;
    var st = store();
    if (!st || typeof st.closeSpace !== 'function') {
      toast('解除失败，模块未就绪');
      return;
    }
    var contact = getContact(cid);
    var charName = contact ? String(contact.name || 'TA') : 'TA';
    confirmDialog({
      title: '解除情侣空间',
      message: '确定要与「' + charName + '」解除情侣空间吗？时光轴、留言板、深夜私语、打卡照片等记录都会被清空，且无法恢复。',
      confirmText: '解除',
      cancelText: '再想想',
      danger: true
    }).then(function (ok) {
      if (!ok) return;
      st.closeSpace(cid);
      var wp = global.miyaCoupleWhisperStore;
      if (wp && typeof wp.removeAllForContact === 'function') wp.removeAllForContact(cid);
      if (state.activeSpaceContactId === cid) state.activeSpaceContactId = '';
      toast('已解除情侣空间');
      setView('gate');
      renderGateList(true);
      if (typeof global.dispatchEvent === 'function' && typeof global.CustomEvent === 'function') {
        global.dispatchEvent(new global.CustomEvent('miya-couple-space-closed', { detail: { contactId: cid } }));
      }
    });
  }
  ...
  function bindEvents() {
    ...
    var unbindBtn = $('cp-unbind-btn');
    if (unbindBtn) unbindBtn.addEventListener('click', function () { unbindSpace(state.activeSpaceContactId); });
