 ...
 * 面板本身不能跟着消失，因为：
 *
 *   1. 桌面上的「生图」图标需要它（点进去就是这套设置 + 自由生图）；
 *   2. 聊天设置里也有一栏「生图 API」指向同一份配置。
 *
 * 于是把它提成独立 App：桌面图标直接打开它，聊天设置里那一栏
 * 也跳到这里，两边读写的是同一份配置，不存在两份状态。
 *
 * ── 一个必须守住的约束 ────────────────────────────────────────
 *
